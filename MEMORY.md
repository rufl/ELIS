...
- Runtime input has explicit focus ownership: focus loss clears held and edge state plus queued text; device lifecycle events still run while suspended; focus return snapshots analog state before `btnp` can observe it. Workshop similarly ignores queued keyboard, text, and pointer events while unfocused.
- Live Workshop commands mutate the project before entering history. They must use `History.commitApplied` with the pre-gesture revision so allocation failure restores data and dirty-state identity; structural snapshots still commit before replacing the authoritative project.
